...
{"@context":"https://schema.org","@type":"Person","name":"Ahmed Ghoneim"}
        ...
        // حل مشكلة عدم ظهور العداد - نعرض رقم فوري أولاً
        let localVisits = parseInt(localStorage.getItem('local_visits') || '0');
        localVisits++;
        localStorage.setItem('local_visits', localVisits);
        totalVisits = baseVisits + localVisits;
        
        // عرض الرقم فوراً بدون انتظار API
        document.addEventListener('DOMContentLoaded', () => {
            const counterEl = document.getElementById('visit-counter');
            if(counterEl) counterEl.innerText = totalVisits.toLocaleString();
        });

        // بعدين نحاول نجيب الرقم الحقيقي من API
        fetch('https://api.counterapi.dev/v1/ahmedghoneim_portfolio/visits/up')
            .then(response => response.json())
            .then(data => { 
                totalVisits = baseVisits + data.count;
                const counterEl = document.getElementById('visit-counter');
                if(counterEl) counterEl.innerText = totalVisits.toLocaleString();
            })
            .catch(error => {
                console.log('API fallback to localStorage');
            });
